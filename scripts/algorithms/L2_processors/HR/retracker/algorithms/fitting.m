function fit_res = fitting(data, cnf_p, chd_p , nf_p, ini_p,varargin)
...
switch cnf_p.range_index_method
        case 'conventional'
            range_index_comm=1:data.N_samples;
        case 'resampling'
...
%--------------------------------------------------------------------------            
        ...
        if cnf_p.wvfm_portion_selec
            if strcmpi(cnf_p.wvfm_portion_selec_type,'ref_height')
                [start_sample,stop_sample,flag_nadir_return_in_win] = wvfm_portion_selec(data.HRM.power_wav_filtered(m,:),data.MEA.win_delay(m),cnf_p,m,...
...
%--------------------------------------------------------------------------
        if ~any(~isnan(data.HRM.power_wav_filtered(m,start_sample:stop_sample)))
            fit_res.Epoch(m)    =   NaN;
            if cnf_p.rou_flag
                fit_res.rou(m)  =   NaN; 
            else
...
%function for computational load issues    
    ...
    case 'internal'
        ...
        nf_params.range_history=compute_range_history_approx(data,nf_p,cnf_p,m); 
        if isfield(data.HRM,'Doppler_mask')
            ...
            nf_params.Doppler_mask_beams_all_zeros_original=logical(ones(1,nf_params.Neff));
        end
...
%% --------------------------- ThN ----------------------------------------
% -------------------------------------------------------------------------
            if cnf_p.Thn_flag && ~cnf_p.fit_noise && ~cnf_p.external_Thn_flag
            ...
            elseif cnf_p.Thn_flag && ~cnf_p.fit_noise && cnf_p.external_Thn_flag
                nf_params.ThN        =ones(1,nf_p.Neff(m)).*cnf_p.external_Thn_value;
            else
...
% -------------------------------------------------------------------------                        
            ...
            if cnf_p.lut_flag
                switch cnf_p.power_wfm_model
                    case 'simple'
                        switch cnf_p.fitting_fun_type
                            case 'lsq'
                                mpfun               =   @(fit_params,x)ml_wave_gen(x, fit_params, nf_params, cnf_p,looks,func_f0);
                            case 'fmin'
                                fminfun               =   @(fit_params)sum((ml_wave_gen(range_index, fit_params, nf_params, cnf_p,looks,func_f0)-fit_data).^2);
                        end
                ...
                switch cnf_p.fitting_fun_type
                    case 'lsq'
                        mpfun               =   @(fit_params,x)ml_wave_gen(x, fit_params, nf_params, cnf_p,looks);
                    case 'fmin'
                        fminfun               =   @(fit_params)sum((ml_wave_gen(range_index, fit_params, nf_params, cnf_p,looks)-fit_data).^2);
                end
            end
...
% -------------------------------------------------------------------------        
            if cnf_p.lut_flag
                switch cnf_p.power_wfm_model
                    case 'simple'
                        ml_wav          =   ml_wave_gen(range_index,fit_params,nf_params, cnf_p,looks,func_f0);
                        
                    case 'complete'
                        ml_wav          =   ml_wave_gen(range_index,fit_params,nf_params, cnf_p,looks,func_f0,func_f1);
                end
            else
                ml_wav          =   ml_wave_gen(range_index,fit_params,nf_params, cnf_p,looks);
            end
...
% -------------------------------------------------------------------------       
            if cnf_p.plot_fits_flag && ((mod(m,cnf_p.plot_fits_downsampling)==0) || (m==1)) && (data.GEO.LAT(m)>=cnf_p.plot_fits_lat_range(1) && data.GEO.LAT(m)<=cnf_p.plot_fits_lat_range(2))
                f1=figure;
                plot(1:data.N_samples,data.HRM.power_wav_filtered(m,:)/max(data.HRM.power_wav_filtered(m,:)),'-b');
                hold on                
                if cnf_p.two_step_fitting && COR_1st_iter<cnf_p.two_step_fitting_COR_threshold_rou/100
                    ...
                    plot(start_sample:stop_sample,ml_wav_2nd_iter,'.-g'); %ice-like 
                    h_leg=legend('L1b-Waveform', 'Analytical fit (SWH)','Analytical fit (Rou)','Location','northeastoutside');
                    ...
                    plot(start_sample:stop_sample,ml_wav, '--r')
                    if cnf_p.rou_flag
...
%                     print('-dpng ',[path_Results,'plots/fitted_waveforms/',L1B_filename(17:47),'_',cnf_p.file_ext_string,'_analytical_',type_fit,'_record_',num2str(m),'.png']);
%                 else
%                     print('-dpng ',[path_Results,'plots/fitted_waveforms/',L1B_filename(17:47),'_analytical_',type_fit,'_record_',num2str(m),'.png']);
%                 end
                print('-dpng ',[path_Results,'plots/fitted_waveforms/',L1B_filename(17:47),'_analytical_',type_fit,'_wvfm_',num2str(m),'.png']);
